...
async def create_event_text(event: EventDTO, state=None, **kwargs) -> str:
    ...
    text += f'👤️ Кол-во участников: {event.max_participants if event.max_participants else 0}\n\n'
    text += f'👥 Кол-во команд: {len(event.teams if event.teams else [])}\n\n'
    if 'status' in state:
        text += '🟢 ' if event.status == EventStatus.CONDUCTED \
            else '🔴 ' if event.status == EventStatus.CANCELED \
            else '🟠 ' if event.status == EventStatus.PAYMENT_WAIT \
            else '🔵 '
        text += f'Статус: {event.status.value}\n\n'
    text += f'💰 Сумма: {event.amount if event.amount else 0} тг.\n\n'

    text += '--------------------\n\n'

    if 'name' in state:
        text += "⚠️ Ведите название мероприятия"
    if 'description' in state:
        text += "⚠️ Ведите описание мероприятия"
    if 'participants_limit' in state:
        text += "⚠️ Укажите максимальное кол-во участников"
    if 'teams_count' in state:
        text += "⚠️ Укажите кол-во команд"
    if 'payment_amount' in state:
        text += "⚠️ Укажите стоимость мероприятия"
    if 'date' in state:
        text += "⚠️ Укажите дату мероприятия в формате DD.MM.YYYY HH:mm"
    if 'links' in state:
        text += "📬 Пригласительные ссылки:\n"
        for index, team in enumerate(event.teams):
            text += f"Команда {index + 1}: 🔑{hlink(f'команда {index + 1}', f'https://t.me/makeurmatch_bot?start={team.id}')}\n"
        text += "\n"
    ...
    if 'payment' in state:
        if kwargs.get('payment_status') != PaymentStatus.PAID:
            ...
            text += f"💳 Ваша ссылка на оплату:\n{hlink('ссылка', kwargs.get('payment_link'))}\n"
        else:
    ...
    if 'canceled_by' in state:
        text += f"❗️ Участник отменивший оплату: {kwargs.get('canceled_by')}\n"
